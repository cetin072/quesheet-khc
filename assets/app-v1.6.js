(()=>{
  ...
  function showNewEventModal(){
    let selected='blank';
    const options=[{key:'blank',label:'빈 큐시트로 시작',description:'순서를 직접 추가합니다.'},...templates];
    const draw=()=>{
      openModal(`<div class="modal-head"><h2>새 행사 만들기</h2><button class="modal-close" id="modalClose">×</button></div><p class="notice">시작 방법을 선택한 뒤 아래 버튼을 누르세요. 기본 양식은 새 행사로 복사되므로 원본 양식은 바뀌지 않습니다.</p><div class="template-grid">${options.map(t=>`<button class="template-card ${selected===t.key?'selected':''}" data-template="${esc(t.key)}"><b>${esc(t.label)}</b><span>${esc(t.description)}</span></button>`).join('')}</div><div class="split-actions" style="margin-top:10px"><button class="template-card" id="copyEventBtn"><b>기존 행사 복제</b><span>저장된 행사를 새 행사로 복사합니다.</span></button></div><div class="modal-actions"><button class="btn" id="cancelCreate">취소</button><button class="btn primary" id="createTemplateBtn">이 양식으로 만들기</button></div>`);
