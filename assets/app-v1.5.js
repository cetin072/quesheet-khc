(()=>{
  ...
  function showNewEventModal(){
    openModal(`<div class="modal-head"><h2>새 행사 만들기</h2><button class="modal-close" id="modalClose">×</button></div><div class="template-grid"><button class="template-card" data-template="blank"><b>빈 행사</b><span>순서를 직접 추가합니다.</span></button><button class="template-card" data-template="opening"><b>개소식</b><span>내빈 소개, 인사말, 축사, 현판식, 촬영 기본 구성</span></button><button class="template-card" data-template="birthday"><b>돌잔치</b><span>돌잡이와 이벤트 중심 구성</span></button><button class="template-card" data-template="copy"><b>기존 행사 복제</b><span>현재 행사 중 하나를 선택해 복사합니다.</span></button></div>`);
    $('#modalClose').onclick=closeModal;document.querySelectorAll('[data-template]').forEach(b=>b.onclick=()=>createFromTemplate(b.dataset.template));
  }
  function createFromTemplate(type){
    if(type==='copy'){showCopyPicker();return;}
    const event={id:uid(),type:type==='opening'?'개소식':type==='birthday'?'돌잔치':'일반 행사',title:type==='opening'?'새 개소식':type==='birthday'?'새 돌잔치':'새 행사',date:today(),time:'',location:'',host:'',notes:'',steps:withIds(type==='opening'?openingSteps:type==='birthday'?birthdaySteps:[]),createdAt:Date.now(),updatedAt:Date.now()};
  ...
  function renderEditor(){
    ...
    const rows=e.steps.map((s,i)=>`<div class="step-row ${s.enabled===false?'off':''}"><span class="step-num">${i+1}</span><div class="step-main"><b>${esc(s.title||'제목 없음')}</b><span>${esc(s.duration||'시간 미정')} · ${s.enabled===false?'사용 안 함':'사용'}</span></div><div class="step-tools"><button title="위로" data-up="${s.id}" ${i===0?'disabled':''}>↑</button><button title="아래로" data-down="${s.id}" ${i===e.steps.length-1?'disabled':''}>↓</button><button class="edit" data-edit="${s.id}">편집</button></div></div>`).join('');
  ...
  function editStep(event,stepId){
    const existing=stepId?stepById(event,stepId):null;const s=existing||{id:uid(),title:'새 순서',duration:'약 1분',script:'',cue:'',contingency:'',enabled:true,completed:false};
    openModal(`<div class="modal-head"><h2>${existing?'순서 편집':'순서 추가'}</h2><button class="modal-close" id="modalClose">×</button></div><div class="form-grid"><div class="field full"><label>순서 제목</label><input id="stepTitle" value="${esc(s.title)}"></div><div class="field"><label>예상 소요시간</label><input id="stepDuration" value="${esc(s.duration||'')}"></div><div class="field"><label>사용 여부</label><select id="stepEnabled"><option value="yes" ${s.enabled!==false?'selected':''}>사용</option><option value="no" ${s.enabled===false?'selected':''}>사용 안 함</option></select></div><div class="field full"><label>사회자 멘트</label><textarea id="stepScript" style="min-height:180px">${esc(s.script||'')}</textarea></div><div class="field full"><label>현장 진행 지시</label><textarea id="stepCue">${esc(s.cue||'')}</textarea></div><div class="field full"><label>돌발상황·대체 멘트</label><textarea id="stepCont">${esc(s.contingency||'')}</textarea></div></div><div class="modal-actions">${existing?'<button class="btn danger" id="removeStep">삭제</button>':''}<button class="btn" id="cancelStep">취소</button><button class="btn primary" id="saveStep">저장</button></div>`);
    $('#modalClose').onclick=$('#cancelStep').onclick=closeModal;$('#saveStep').onclick=()=>{s.title=$('#stepTitle').value.trim()||'제목 없음';s.duration=$('#stepDuration').value.trim();s.script=$('#stepScript').value;s.cue=$('#stepCue').value;s.contingency=$('#stepCont').value;s.enabled=$('#stepEnabled').value==='yes';if(!existing)event.steps.push(s);event.updatedAt=Date.now();save();closeModal();renderEditor();toast('저장했습니다.');};
